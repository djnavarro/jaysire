...
page1a <- timeline(followup) %>%
  tl_display_if(data_condition(button_pressed == "0"))
```
...
Next, I'll expand on what is going on with the `data_condition()` function, but to do so I need to explain the `tl_display_if()` function in a little more detail. First, let's rewrite the code above without using the pipe, and name our arguments explicitly:
...
page2 <- timeline(query) %>%
  tl_display_while(data_condition(button_pressed != "2"))
```
