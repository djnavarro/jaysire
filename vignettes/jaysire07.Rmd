...
```{r}
page1a <- timeline(followup) %>%
  display_if(data_condition(button_pressed == "0"))
```

What we have done here is defined `page1a` as a *conditional* timeline. When the participant actually completes the experiment, this trial will *only* be shown if the condition `data_condition(button_pressed == "0")` is true. This solves our problem, but it may not be obvious *how* we're solving the problem, so it is worth unpacking this a little...
...
Next, I'll expand on what is going on with the `data_condition()` function, but to do so I need to explain the `display_if()` function in a little more detail. First, let's rewrite the code above without using the pipe, and name our arguments explicitly:

```{r}
page1a <- display_if(
  timeline = timeline(followup), 
  conditional_function = data_condition(button_pressed == "0")
)
```

This isn't quite so pretty to look at, but it is helpful for the purposes of understanding. The `display_if()` function takes two arguments, the `timeline` object itself (i.e., the `followup` page), and a `conditional_function` that is used to determine whether or not the timeline should be executed. Because the experiment is eventually run in javascript through the web browser, this function must be a *javascript* function, not an R function. If you know javascript, then it may be useful to note that jaysire contains a `code()` function that means that any text you include will be passed as unfiltered javascript, so you can in fact pass anything you like here:

```{r eval=FALSE}
page1a <- display_if(
  timeline = timeline(followup), 
  conditional_function = code("/* your javascript function here */")
)
```
...
Now what we do is take this `query` trial, wrap it as a `timeline()` and then keep repeating that trial until the user responds by pressing button `"2"` (i.e., selects `"Pleasant"`):

```{r looping}
page2 <- timeline(query) %>%
...
Let's wrap all this up in a single timeline, add the resources, and build it as an experiment:

```{r}
experiment(
  timeline = timeline(page1, page1a, page2),
  resources = add_resources(resources),
  path = file.path(tempdir(), "flow_exp"), 
