...
Now that we have specified the stimuli as an ordinary R variable, what we need to do is to link it to the trial template we defined earlier. To do this, we first "wrap" `trial_template` into a timeline, and use the `tl_add_variables()` function to attach a variable to that timeline:
