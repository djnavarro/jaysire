...
Now that we have specified the stimuli as an ordinary R variable, what we need to do is to link it to the trial template we defined earlier. To do this, we first "wrap" `trial_template` into a timeline, and use the `with_variables()` function to attach a variable to that timeline:

```{r}
trials <- timeline(trial_template) %>%
  with_variables(my_stimulus = equations)
```
...
At this point we are done, more or less. We'll still have to do the boring job of creating a final trial that tells the participant that the experiment is complete, and of course we'll have to use the `experiment()` function to actually build the experiment! Once we've done that, we end up with the following as the complete code for the experiment:
