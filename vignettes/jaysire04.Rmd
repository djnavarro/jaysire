...
This is familiar to us from the previous article. I've specified the `stimulus` itself using the `resource()` function, listed the `choices` available to participants, and some ancillary information about the size of the image. 
...
video_trial <- trial_video_button_response(
  sources = resource(c("heart.mpg", "heart.webm")), 
  choices = c("Unpleasant", "Neutral", "Pleasant"), 
...
There is one last thing worth discussing, namely the file structure of the experiment that we have build. To start with, let's take a look at what `add_resources()` creates:
