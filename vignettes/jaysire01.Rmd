...
Now that we have our trial objects, `instructions`, `trial1` and `trial2`, our next taks is to bind together into a timeline. The `timeline()` function allows us to do this:
...
At the moment we have a complete timeline, but it is stored in an abstract form as the `all_trials` variable. What we really want to do is "build" an experiment from this timeline: we want to write the files that will run the experiment, and save those files somewhere. That is the job of the `experiment()` function. First, let's specify the location of the experiment:
...
When we run the `experiment()` function, two subfolders within the `exp_path` folder are created, one called "data" (which is initially empty) and another called "experiment", which contains all the source files required to run the experiment. Here are the files we've just created:
...
experiment(
  timeline = timeline(all_trials, all_done),
  path = exp
)
```
